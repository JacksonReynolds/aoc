...
const [times = [], records = []] = readLines(6)
  .filter((l) => !!l.trim())
  .map((lineRaw) =>
    lineRaw
      .split(":")[1]
...
const getPathsToGlory = (timeLimit: number, recordToBeat: number) => {
  const pathsToGlory = [];
  for (let timeHeld = 1; timeHeld < timeLimit; timeHeld++) {
    const cruisingSpeed = timeHeld;
    const remainingTime = timeLimit - timeHeld;
    const distance = cruisingSpeed * remainingTime;

    if (distance > recordToBeat) pathsToGlory.push(timeHeld);
  }

  return pathsToGlory.length;
};

const partOne = () => {
  let product = 1;
  for (let i = 0; i < times.length; i++) {
    product *= getPathsToGlory(times[i], records[i]);
  }
...
const partTwo = () => {
  const [time, record] = [times, records].map((arr) => Number(arr.join("")));
  return getPathsToGlory(time, record);
};
