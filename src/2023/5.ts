...
const seeds = seedsRaw[0].split(" ").map(Number);
const almanac = mapsRaw.map((map) =>
  map.map((entry) => entry.split(" ").map(Number)),
);

const processSeedLocation = (source: number) => {
  for (const map of almanac) {
    const entry = map.find(
      ([_destinationRangeStart, sourceRangeStart, rangeLength]) =>
        source > sourceRangeStart && source < sourceRangeStart + rangeLength,
    );
    if (entry) {
      const [destinationRangeStart, sourceRangeStart, _rangeLength] = entry;
      source = destinationRangeStart + source - sourceRangeStart;
    }
  }
...
const seedRanges = () => {
  ...
  for (let i = 0; i < seeds.length; ) {
    ranges.push({ start: seeds[i], interval: seeds[i + 1] });
    i += 2;
  }
  return ranges;
};
const seedExists = (seed: number) =>
  seedRanges().some(
    ({ start, interval }) => start <= seed && seed <= start + interval,
  );

const processLocationSeed = (destination: number) => {
  for (const map of almanac.reverse()) {
    const entry =
      map.find(
        ([destinationRangeStart, _sourceRangeStart, rangeLength]) =>
          destination >= destinationRangeStart &&
          destination < destinationRangeStart + rangeLength,
      ) ?? [];
    if (entry) {
      const [destinationRangeStart, sourceRangeStart, _rangeLength] = entry;
      destination = sourceRangeStart + destination - destinationRangeStart;
    }
  }
  return destination;
};
...
const partOne = () => {
  return Math.min(...seeds.map(processSeedLocation));
};

const partTwo = () => {
  for (let i = 0; i < Number.MAX_SAFE_INTEGER; i++) {
    const seed = processLocationSeed(i);

    if (seedExists(seed)) return i;
  }
};
