...
const lines = readLines(4).filter((l) => !!l.trim());
...
const processCard = (line: string) => {
  const endOfTitle = line.indexOf(":");
  const [cardTitle, winningNumsStr, playerNumsStr] = [
    line.substring(0, endOfTitle),
    ...line.substring(endOfTitle + 1).split("|"),
  ];
  const cardId = +cardTitle.replace(/[^0-9]/g, "");
  const winningNums = (winningNumsStr.match(/[0-9]+/g) ?? []).map(Number);
  const playerNums = (playerNumsStr.match(/[0-9]+/g) ?? []).map(Number);
  const matchingNums = playerNums.filter((playerNum) =>
    winningNums.includes(playerNum),
  );
...
const partOne = (data = lines) =>
  data.reduce<number>((runningTotal, rawCard) => {
    if (!rawCard.trim()) return runningTotal;
    const { score } = processCard(rawCard);
...
const partTwo = (data = lines) => {
  ...
  data.forEach((rawCard, i) => {
    ...
    Array.from({ length: copies }).forEach(() => {
      matchingNums.forEach((_, j) => {
        if (i + j + 1 > cardCounts.size) return;
        cardCounts.set(i + j + 1, (cardCounts.get(i + j + 1) ?? 0) + 1);
      });
    });
  });

  return Array.from(cardCounts.values()).reduce(
    (runningSum, copiesCount) => runningSum + copiesCount,
    0,
  );
};
